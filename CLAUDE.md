...
- 검색엔진 소유확인: 네이버 `3cbdd137c1ac1afae84c69d9917bfee998256ff1` · 구글 `mXbgogfOxuqGFD79qg1Qi2M8LIohYFwAYe3W6KWHLv8` — 홈 `<head>` 에 생성기가 넣는다(`regions.js` 의 `verify`). **`index.html` 에 손으로 넣지 말 것** — 재생성하면 사라진다. 코드를 바꾸려면 `verify` 를 고치고 재생성.
